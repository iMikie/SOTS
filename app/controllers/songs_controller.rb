...
# Never trust parameters from the scary internet, only allow the white list through.
  def song_params
    params.require(:song).permit(:title, :voicing, :composer, :arranger_one, :arranger_two, :category, :larger_work, :song_number, :pdf_file, :mp3_file, :t1mp3_file, :t2mp3_file, :b1mp3_file, :b2mp3_file, :performance_id)
  end
...
#https://devcenter.heroku.com/articles/s3
  ...
  def handle_pdf_mp3(song_hash)
    ...
    t1mp3_filename = path.to_s + "T1.mp3"
    t2mp3_filename = path.to_s + "T2.mp3"
    b1mp3_filename = path.to_s + "B1.mp3"
    b2mp3_filename = path.to_s + "B2.mp3"

    pdf_file_object = song_hash[:pdf_file]
    mp3_file_object = song_hash[:mp3_file]
    t1mp3_file_object = song_hash[:t1mp3_file]
    t2mp3_file_object = song_hash[:t2mp3_file]
    b1mp3_file_object = song_hash[:b1mp3_file]
    b2mp3_file_object = song_hash[:b2mp3_file]
    ...
    if !t1mp3_file_object.nil?
      File.open(t1mp3_filename, 'wb') do |f|
        f.write t1mp3_file_object.read
        write_Amazon_file song + "T1.mp3", t1mp3_filename
      end
    end

    if !t2mp3_file_object.nil?
      File.open(t2mp3_filename, 'wb') do |f|
        f.write t2mp3_file_object.read
        write_Amazon_file song + "T2.mp3", t2mp3_filename
      end
    end

    if !b1mp3_file_object.nil?
      File.open(b1mp3_filename, 'wb') do |f|
        f.write b1mp3_file_object.read
        write_Amazon_file song + "B1.mp3", b1mp3_filename
      end
    end

    if !b2mp3_file_object.nil?
      File.open(b2mp3_filename, 'wb') do |f|
        f.write b2mp3_file_object.read
        write_Amazon_file song + "B2.mp3", b2mp3_filename
      end
    end
    ...
    song_hash.delete(:t1mp3_file) #remove the filename from the hash
    song_hash.delete(:t2mp3_file) #remove the filename from the hash
    song_hash.delete(:b1mp3_file) #remove the filename from the hash
    song_hash.delete(:b2mp3_file) #remove the filename from the hash
  end
